...
public class InvoiceServiceImp implements InvoiceService {
    ...
    public InvoiceResponse create(InvoiceRequest invoiceRequest) {
        if (invoiceRequest.getPayType().equals(PayType.MOMO)) {
            ...
            try {
                QueryStatusTransactionResponse queryStatusTransactionResponse = QueryTransactionStatus
                        .process(environment, invoiceRequest.getOrderIdMomo(), invoiceRequest.getRequestIdMomo());
                System.out.println("qqqq-----------------------------------------------------------"
                        + queryStatusTransactionResponse.getMessage());
        ...
        invoice.setAddress(userAddress.get().getStreetName() + ", " + userAddress.get().getWards().getName() + ", "
                + userAddress.get().getWards().getDistricts().getName() + ". "
                + userAddress.get().getWards().getDistricts().getProvince().getName());
    ...
    public InvoiceResponse cancelInvoice(Long invoiceId) {
        ...
        // if(idSt == StatusUtils.DA_GUI || idSt == StatusUtils.DA_NHAN || idSt ==
        // StatusUtils.DA_HUY || idSt == StatusUtils.KHONG_NHAN_HANG){
        // throw new MessageException(invoice.get().getStatus().getName()+ " không thể
        // hủy hàng");
        // }
        ...
        // i.getProductSize().setQuantity(i.getQuantity() +
        // i.getProductSize().getQuantity());
